...
def save_clinical_record(o_id, patient_name, scan_path, heatmap_path, dr_class, confidence_str):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute("SELECT p_id FROM patients WHERE o_id=? AND patient_name=?", (o_id, patient_name))
    patient = cursor.fetchone()
    if not patient:
        cursor.execute("INSERT INTO patients (o_id, patient_name) VALUES (?, ?)", (o_id, patient_name))
    ...
    cursor.execute("""
        INSERT INTO scans (p_id, original_image_path, heatmap_path, ai_predicted_class, ai_confidence)
        VALUES (?, ?, ?, ?, ?)
    """, (p_id, scan_path, heatmap_path, dr_class, conf_decimal))
    conn.commit()
    conn.close()
...
def get_doctor_history(o_id):
    ...
    cursor.execute("""
        SELECT s.scan_id, p.patient_name, s.scan_date, s.original_image_path, s.heatmap_path, 
               s.ai_predicted_class, s.ai_confidence, s.doctor_verified_class
        FROM scans s
        JOIN patients p ON s.p_id = p.p_id
        WHERE p.o_id = ?
...
# ==========================================
    ...
    dash_tab, history_tab = st.tabs(["New Patient Scan", "Patient History & Retraining"])
    ...
    with history_tab:
        st.markdown("### Clinic History & AI Review")
        ...
        if not history:
            ...
            for record in history:
                scan_id, p_name, date, orig_img, heat_img, ai_class, ai_conf, doc_class = record
                ...
                with st.expander(f"Patient: {p_name} | Date: {date[:10]} | AI predicted: Class {ai_class}"):
                    ...
                    if doc_class is None:
                        st.warning("Needs Doctor Verification")
                        with st.form(key=f"form_{scan_id}"):
                        ...
                        if doc_class == ai_class:
                            ...
                            st.write(f"AI was **Incorrect** (AI Guessed: {ai_class}). Saved to retraining pipeline.")